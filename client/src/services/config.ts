export const config = {
  ...
  // V20: Approved validators list (comma-separated public keys)
  approvedValidators: (process.env.REACT_APP_APPROVED_VALIDATORS || '').split(',').filter(v => v.length > 0),
  // V20: Min stake 1 CSPR (pool-based, no direct delegation minimum)
  minStakeAmount: process.env.REACT_APP_MIN_STAKE_AMOUNT || '1000000000',
};
